...
from Decode.NNDecoder import SingleLabelNNDecoder0, SingleLabelNNDecoder1, SingleLabelNNDecoder2, MultiLabelNNDecoder1, MultiLabelNNDecoder2, MultiLabelNNDecoder3
...
def SLNN_training0(snr, method, nr_codeword, bits, encoded, epochs, learning_rate, momentum, batch_size, model_save_path, model_name, NN_type, patience, delta, device):
    ...
    model = SingleLabelNNDecoder0(input_size, output_size).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(SLNN_trainloader, 0):
            ...
            if i % 1000 == 999:  # Print every 100 mini-batches
                print(f'{NN_type} New: SNR{snr_measure}, Epoch {epoch + 1}, Batch {i + 1}, Loss: {running_loss / 1000:.9f}')
                running_loss = 0.0
        ...
        print(f'{NN_type} New Testing: - SNR{snr_measure} - Loss: {running_loss / len(SLNN_testloader):.9f}')
        ...
        if early_stopping(running_loss, model, model_save_path, model_name):
            ...
            print(f'{NN_type} New: SNR={snr_measure} Stop at total val_loss is {running_loss/len(SLNN_testloader)} and epoch is {epoch}')
            break
        else:
...
def main():
    ...
    for NN_type in NeuralNetwork_type:
        ...
        if NN_type == "SLNN" :
            model_name = f"{NN_type}_hiddenlayer0"
            SLNN_training0(snr, encoding_method, nr_codeword, bits, encoded, epochs, learning_rate, momentum,
                           batch_size, model_save_path, model_name, NN_type, patience, delta, device)
        ...
        if NN_type == "MLNN":
            ...
            # for hidden_size in MLNN_hidden_size1:
            #     model_name = f"{NN_type}_hiddenlayer{hidden_size}"
            #     MLNN_training1(snr, encoding_method, nr_codeword, bits, encoded, epochs, learning_rate, momentum, batch_size, hidden_size,
            #                    model_save_path, model_name, NN_type, patience, delta, device)

            for hidden_size in MLNN_hidden_size2:
